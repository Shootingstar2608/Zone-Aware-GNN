...
# ///////////////////////////////////////////////////////////////////
    ...
    def forward(self, Z: torch.Tensor, time_idx: torch.Tensor) -> torch.Tensor:
        ...
        time_idx: (B,)
        → (B, N, d_z) — dense zone embedding (batch dependent)
        """
        B = time_idx.size(0)
        N = Z.size(0)
        ...
        t_batch = t_emb.unsqueeze(1).expand(B, N, -1)
        fused = torch.cat([Z_batch, t_batch], dim=-1)
...
# ///////////////////////////////////////////////////////////////////
        ...
        # Adaptive adjacency
# ///////////////////////////////////////////////////////////////////
        score = torch.bmm(torch.bmm(E_zone, W), E_zone.transpose(1, 2))  # (B,N,N)
# ///////////////////////////////////////////////////////////////////
...
# ///////////////////////////////////////////////////////////////////
        z_embed: (B, N, d_z) zone embedding
# ///////////////////////////////////////////////////////////////////
        → H':    (B, N, out_channels)
        """
...
# ///////////////////////////////////////////////////////////////////
        B = H.size(0)
        E_batch = E.unsqueeze(0).expand(B, -1, -1)       # (B, N, d_e)
        fused = torch.cat([E_batch, z_embed], dim=-1)    # (B, N, d_e+d_z)
        W_v   = self.W_gen(fused).view(B, self.N, self.in_ch, self.out_ch)  # (B,N,Fin,Fout)
        b_v   = self.b_gen(fused)                        # (B, N, Fout)

        # Node-specific transformation: (B,N,Fin) × (B,N,Fin,Fout) → (B,N,Fout)
        H_transformed = torch.einsum('bni,bnio->bno', H, W_v)
# ///////////////////////////////////////////////////////////////////
...
# ///////////////////////////////////////////////////////////////////
            TimeZoneModulatedGraphConv(in_channels, hidden_channels,
# ///////////////////////////////////////////////////////////////////
        ...
        for _ in range(num_layers - 1):
            self.conv_layers.append(
# ///////////////////////////////////////////////////////////////////
                TimeZoneModulatedGraphConv(hidden_channels, hidden_channels,
# ///////////////////////////////////////////////////////////////////
                                       num_nodes, node_embed_dim, zone_embed_dim)
            )
    ...
    def forward(
        ...
        """
# ///////////////////////////////////////////////////////////////////
        # 1. Zone embedding (phụ thuộc vào time_idx)
...
# ///////////////////////////////////////////////////////////////////
    print(f"   Zone embedding output: {model.zone_emb(Z, time_idx).shape}")

    # Kiểm tra zone-awareness: 2 nodes với zone khác nhau có W_v khác nhau?
    with torch.no_grad():
        z_emb = model.zone_emb(Z, time_idx)
        E_shared = model.adj_module.E
        E_batch = E_shared.unsqueeze(0).expand(B, -1, -1)
        fused = torch.cat([E_batch, z_emb], dim=-1)
        W_nodes = model.conv_layers[0].W_gen(fused)
    print(f"\n📊 Node weight diversity (std across nodes): {W_nodes.std(dim=1).mean():.4f}")
# ///////////////////////////////////////////////////////////////////
